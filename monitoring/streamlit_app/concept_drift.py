...
if reference.empty or current_data.empty:
    ...
    report.run(reference_data=reference, current_data=current_data, reference_target=np.zeros(len(reference)), current_target=current_labels)
    ...
    # Calculate AUC and send email if below threshold
    from sklearn.metrics import roc_auc_score

    if len(current_labels) > 0 and "prediction" in user_preds[0]:
        predictions = np.array([p["prediction"] for p in user_preds])
