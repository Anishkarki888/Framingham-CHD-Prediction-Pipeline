...
html_report_file = os.path.join(REPORTS_PATH, "data_drift_report.html")
dashboard.save(html_report_file)
st.write(f"✅ Data Drift report saved to HTML: {html_report_file}")

# --------------------------
# Save dashboard object as pickle
# --------------------------
pkl_report_file = os.path.join(REPORTS_PATH, "data_drift_report.pkl")
with open(pkl_report_file, "wb") as f:
    pickle.dump(dashboard, f)
st.write(f"✅ Data Drift report saved to PKL: {pkl_report_file}")

# Show HTML in Streamlit
st.components.v1.html(open(html_report_file, "r").read(), height=800, scrolling=True)
